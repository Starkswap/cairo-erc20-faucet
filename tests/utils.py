...
class Account(Signer):
    ...
    async def mint(self, erc20_contract, to_contract, amount:int):
        ...
        return await self.send_transaction(to=erc20_contract.contract_address, selector_name='mint', calldata=[to_contract.contract_address, *uint_amount])
    ...
    async def transfer(self, erc20_contract, to_user, amount:int):
        uint_amount = uint(amount)
        return await self.send_transaction(to=erc20_contract.contract_address, selector_name='transfer', calldata=[to_user.contract_address, *uint_amount])

    async def approve(self, erc20_contract, spender_address, amount:int):
        uint_amount = uint(amount)
        return await self.send_transaction(to=erc20_contract.contract_address, selector_name='approve', calldata=[spender_address, *uint_amount])

    async def send_transaction(self, to, selector_name, calldata, nonce=None):
        return await super(Account, self).send_transaction(account=self.contract, to=to, selector_name=selector_name, calldata=calldata, nonce=nonce)
